...
const getMatches = async () => {
    ...
    let date = `${year}-${month.toString().length === 1 ? `0${month + 1}` : month + 1}-${day.toString().length === 1 ? `0${day}` : day}`
    const res = await fetch(`https://api.football-data.org/v2/matches?competitions=2021,2014,2019,2002&dateFrom=${date}&dateTo=${date}&status=SCHEDULED`,
        ...
        // const res = await fetch(`https://api.football-data.org/v2/matches?dateFrom=2020-09-11&dateTo=2020-09-11`,
...
function writeTweet(matches) {
    let msg = `Today's ${getTwitterHandle(matches[0].competition.name)} matches\r\n\r\n`;
    let lastTime = '';
    matches.forEach(match => {
        const newTime = new Date(match.utcDate);
        let matchTime = newTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        // let matchTime = newTime.toLocaleTimeString('en-GB', { timeZone: 'Europe/London', hour: '2-digit', minute: '2-digit' });
        if (lastTime != matchTime) { msg += `\r\n⏰ ${matchTime}\r\n` }
